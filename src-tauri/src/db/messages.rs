    ...
    }
    {
        let mut stmt = tx.prepare(
            "INSERT INTO messages
                (account_id, uid, message_id, headers, body, mirror_state, received_at,
...
pub fn thread_messages(conn: &Connection, thread_id: &str) -> Result<Vec<MessageDetail>> {
    let mut stmt = conn.prepare(
        "SELECT id, account_id, from_name, from_addr, subject, headers, body,
                received_at, seen, mirror_state, uid
         FROM messages WHERE thread_id = ?1 ORDER BY received_at ASC",
    )?;
...
pub fn set_body(conn: &Connection, message_id: i64, body: &str) -> Result<()> {
    conn.execute(
        "UPDATE messages SET body = ?1, mirror_state = 'full' WHERE id = ?2",
